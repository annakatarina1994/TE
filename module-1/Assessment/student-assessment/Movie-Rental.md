...
4. Instantiate an object, or objects, in the Module1CodingAssessment class inside the `main()`/`Main()` method, and use the object(s) to test your methods by printing out the title, format and rental price of that object.
...
6. Override the `toString()`/`ToString()` method and have it return `"MOVIE - {title} - {format} - {rental price}"` where `{title}`, `{format}`, and `{rental price}` are placeholders for the actual values. The values from the object should be shown in the string where `{variable-name}` is indicated. Demonstrate this works but creating another object and printing using the toString.
