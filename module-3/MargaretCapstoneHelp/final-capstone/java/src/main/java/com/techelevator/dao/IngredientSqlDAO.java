...
public class IngredientSqlDAO implements IngredientDAO {
	...
	@Override
	public void addNewIngredientToExistingRecipe(IngredientWithUser ingredientObj, int recipeId) {
		Long userId = ingredientObj.getUserId();
		for (Ingredient ingredient: ingredientObj.getIngredients()) {
		...
		String sql = "INSERT INTO ingredients (user_id, name, is_on_grocery_list, recipe_id, api_recipe_id, quantity, measurement) VALUES (?, ?, ?, ?, ?, ?, ?)";
		 template.update(sql, userId, ingredient.getName(), ingredient.isOnGroceryList(), recipeId, ingredient.getApiRecipeId(), ingredient.getQuantity(), ingredient.getMeasurement());
		}
	}
...
//	}
	...
	private Ingredient mapResultsToIngredients(SqlRowSet results){
		...
		int apiRecipeId = results.getInt("api_recipe_id");
		int quantity = results.getInt("quantity");
		String measurement = results.getString("measurement");

	Ingredient ingredient = new Ingredient(ingredientId, userId, name, isOnGroceryList, recipeId, apiRecipeId, quantity, measurement);
