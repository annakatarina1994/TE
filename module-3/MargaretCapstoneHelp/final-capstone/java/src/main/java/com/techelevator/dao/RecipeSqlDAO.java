...
public class RecipeSqlDAO implements RecipeDAO {
	...
	public void addNewRecipe(Recipe recipe) {
		String addRecipeSql = "INSERT INTO recipes (api_recipe_id, user_id, title, instructions) "+
	" VALUES (?, ?, ?, ?)";
		template.update(addRecipeSql, recipe.getApiRecipeId(), recipe.getUserId(), recipe.getTitle(), recipe.getInstructions() );
	}

	@Override
	public void editRecipe(Recipe recipe) {
		String editRecipeSql = "UPDATE recipes SET api_recipe_id = ?, user_id=?, title=?, instructions=? WHERE recipe_id = ?";
		template.update(editRecipeSql, recipe.getApiRecipeId(), recipe.getUserId(), recipe.getTitle(), recipe.getInstructions(), recipe.getRecipeId());
	}
	...
	private Recipe mapResultsToRecipe(SqlRowSet result) {
		...
		int apiRecipeId = result.getInt("api_recipe_id");
		int userId = result.getInt("user_id");
		String title = result.getString("title");
		String instructions = result.getString("instructions");
		
		Recipe recipe = new Recipe (id, apiRecipeId, userId, title, instructions);
		return recipe;
	}
