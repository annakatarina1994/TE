...
function titleCase(title, exceptionWords){
    let words = title.toLowerCase().split(' '); // gives us an array of the title words
    let exceptions = [];  // gives us an array of the exception words
    if (exceptionWords !== undefined){
        exceptions = exceptionWords.toLowerCase().split(' ');
    }
    let returnTitle = '';
    for (let i = 0; i < words.length; i++){
        ...
        if (isFound && i !== 0 ){ // keep the word in all lower case because it is an exception word
            returnTitle += words[i] + ' ';
        }
        else {
            returnTitle += toTitleCase(words[i]) + ' ';
        }
 ...
 * and makes all other letters lower case
 */
function toTitleCase(word){
    word = word.trim();
    word = word.toLowerCase();
    let firstLetter = word.charAt(0).toUpperCase();
    word = word.substring(1);  // gets rid of the first character
