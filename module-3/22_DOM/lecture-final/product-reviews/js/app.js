...
function setPageTitle() {
  ...
  const pageTitleName = pageTitle.querySelector('.name');
  pageTitleName.innerText = name;
}
...
function displayReviews() {
  ...
  reviews.forEach(
    (review) => {
        ...
        // add each name beforeend means inside the closing tag (main)
...
function addRating(parent, numberOfStars) {
  ...
  // add the class attribute to the div container
  rating.setAttribute('class', 'rating');
  // create a loop that will add img rating number of times
  for (let i = 0; i < numberOfStars; i++){
    const starImg = document.createElement('img');
    starImg.setAttribute('class', 'ratingStar');
    starImg.src = 'img/star.png';
    // append as a child of rating a starImg element
    rating.appendChild(starImg);
  }

  //review div class is the parent and
  // append as a child of the review div class
  // the rating element
