...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const desc = document.querySelector('.description'); // class description
  desc.addEventListener('click', (event) => {
    toggleDescriptionEdit(event.target);  // event.target is the element clicked
  });

  //When user modifies the description and hits enter, save; escape leaves w/o saving
  ...
  btnSaveReview.addEventListener('click', (event) => {
    event.preventDefault();  // stop the page from refreshing after new review
    saveReview();
  })
});
/**
...
function toggleDescriptionEdit(desc) {
  ...
  textBox.classList.remove('d-none');  //d-none bootstraps display-
