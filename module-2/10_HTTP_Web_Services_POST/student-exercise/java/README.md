...
* There are two helper methods you need to write. 
        One makes an `Auction` object given a CSV string containing either three or four elements. 
        The second creates an `HttpEntity` with a content-type header set to JSON.
...
## Step 5: Write the helper methods makeAuction and makeEntity

The makeAuction method must create a new auction from a CSV string. The CSV string may contain either three or four elements.
...
## Step 7: Update an existing auction
