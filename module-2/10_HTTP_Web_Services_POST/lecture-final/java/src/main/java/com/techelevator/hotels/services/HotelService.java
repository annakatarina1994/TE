...
public class HotelService {
  ...
  public Reservation addReservation(String newReservation) {
    ...
    // if user sent in invalid string data, return null
    if (reservation == null) {
    	return null;
    }
    ...
    try {
    	...
    	console.printError("Uh oh!  " + ex.getRawStatusCode() + ": " +
    			ex.getStatusText());
    }  catch (ResourceAccessException ex) {
    	console.printError("Uh oh!!  " + ex.getMessage());
    }
    return reservation;
  }
  ...
  private Reservation makeReservation(String CSV) {
    ...
    if (parsed.length == 5) {
      ...
      // randomly generates a new reservation id.
