...
SELECT c.name, ci.name
   FROM country c
   LEFT JOIN city ci ON c.capital = ci.id;  -- left table is the one listed first
...
SELECT first_name, 'Actor' AS source_table
   FROM actor
UNION
SELECT first_name, 'Customer' AS source_table
   FROM customer
ORDER BY first_name;