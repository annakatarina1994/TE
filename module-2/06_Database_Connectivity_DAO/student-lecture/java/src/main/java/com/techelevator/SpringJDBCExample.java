...
public class SpringJDBCExample {
	...
	public static void main(String[] args) {
		...
		String sqlCreateActor = "INSERT INTO actor(actor_id, first_name, last_name) "+
								"VALUES (?, ?, ?)"; //we would usually not pass in the actor_id and just let postgresql assign it
		
//		jdbcTemplate.update(sqlCreateActor, 1000, "Minnie", "Mouse");
		 ...
		 * a new record.
		 */
		String sqlGetNextId = "SELECT nextval('seq_city_id')"; // not only returns next city_id but also advances the id
		results = worldJdbcTemplate.queryForRowSet(sqlGetNextId); // will return the next city_id from postgres world DB
