...
public class JDBCCityDAOIntegrationTest {
	...
	public void returns_cities_by_country_code() {
		...
		assertNotNull(results);  // we actually get back some data
		assertEquals(1, results.size());  //we should get back only 1 city
		City savedCity = results.get(0);  //0th will contain the city
		assertCitiesAreEqual(theCity, savedCity);
	}

	@Test
	public void returns_multiple_cities_by_country_code() {
		dao.save(getCity("SQL Station", "Montana", TEST_COUNTRY, 65535));
		dao.save(getCity("Postgres Point", "Alaska", TEST_COUNTRY, 42));
	...
	public void returns_cities_by_district() {
		City theCity = getCity("SQL Station", "Tech Elevator", TEST_COUNTRY, 65535);
		dao.save(theCity);
		...
		City savedCity = results.get(0);  //0th will contain the city back from database
		assertCitiesAreEqual(theCity, savedCity);
	}
